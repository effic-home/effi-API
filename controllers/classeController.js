...
exports.getAllClasses = function(req, res) {
    ...
    sql.query(requete, function (error, results) {
        if(error) {
            ...
            if(results == "") {
                res.status(400).send({ error:true, message: "Sorry there are no classes" });
            } else {
                res.send(results);
            }
        }
    ...
    sql.query(requete, req.body, function (error, results) {
        if(error) {
            ...
            if(results.affectedRows == 0) {
                res.status(400).send({ error:true, message: "Sorry we can't create this classe" });
            } else {
...
exports.getInfoClasse = function(req, res) {
    ...
    sql.query(requete, function (error, results) {
        if(error) {
            ...
            if(results == "") {
                res.status(400).send({ error:true, message: "Sorry there is no class" });
            } else {
                res.send(results);
            }
        }
...
exports.updateClasse = function(req, res) {
    ...
    sql.query(requete, req.body, function (error, results) {
        if(error) {
            ...
            if(results.affectedRows == 0) {
                res.status(400).send({ error:true, message: "Sorry we can't update this class" });
            } else {
...
exports.deleteClasse = function(req, res) {
    ...
    sql.query(requete, function (error, results) {
        if(error) {
            ...
            if(results.affectedRows == 0) {
                res.status(400).send({ error:true, message: "Sorry we can't delete this class" });
            } else {
